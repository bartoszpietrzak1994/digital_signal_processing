...
public class SignalParametersException extends DigitalSignalProcessingException
{
    ...
    {
        return new SignalParametersException(DigitalSignalProcessingErrorCode.CALCULATION_DATA_NOT_PROVIDED, String
				.format("Calculation not possible. Expected parameters were not provided. Expected parameters: %s",
						expectedParameters));
    }

    public static SignalParametersException signalTypeNotSupported(String signalType)
    {
        return new SignalParametersException(DigitalSignalProcessingErrorCode.SIGNAL_TYPE_NOT_SUPPORTED, String
				.format("Signal of type %s is not supported.", signalType));
    }

    public static SignalParametersException operationTypeNotSupported(String operationType)
    {
        return new SignalParametersException(DigitalSignalProcessingErrorCode.OPERATION_TYPE_NOT_SUPPORTED, String
				.format("Operation of type %s is not supported", operationType));
    }

    public static SignalParametersException unableToCalculatePeriodNumber(Complex sample)
    {
        return new SignalParametersException(DigitalSignalProcessingErrorCode.UNABLE_TO_CALCULATE_PERIOD_NUMBER,
    ...
    {
        return new SignalParametersException(DigitalSignalProcessingErrorCode.SIGNAL_SAMPLES_NOT_AVAILABLE, "Unable " +
				"to calculate signal values - samples are not available.");
    }
